...
class GridWorldEnv10_X(gym.Env):
    ...
    def __init__(self,render_mode=None,size=4,exploration_max=0.90,exploration_min=0,exploration_decay=1.0,gamma=1,max_steps=18,learning_rate=1):
        ...
        self.reward_matrix = np.array([[0 for _ in range(self.size)] for _ in range(self.size)])
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "closest_left": spaces.Discrete(3),
                "closest_right": spaces.Discrete(3),
                "closest_up": spaces.Discrete(3),
                "closest_down": spaces.Discrete(3),
            }
        )
    ...
    def _get_obs(self):
        return {"agent": self._agent_location, "mean_left": self.mean_left, "mean_right": self.mean_right, "mean_up": self.mean_up, "mean_down": self.mean_down,"closest_left": self.closest_left,"closest_right": self.closest_right,"closest_up": self.closest_up,"closest_down": self.closest_down}
    def _get_info(self):
    ...
    def position_to_left(self, position, reach):
        ...
        for i in range(reach):
            if (pos[0] == lower_bound):
                pos = np.array([upper_bound, pos[1]])
    ...
    def position_to_right(self, position, reach):
        ...
        for i in range(reach):
            if (pos[0] == upper_bound):
                pos = np.array([lower_bound, pos[1]])
    ...
    def position_to_up(self, position, reach):
        ...
        for i in range(reach):
            if (pos[1] == upper_bound):
                pos = np.array([pos[0], lower_bound])
    ...
    def position_to_down(self, position, reach):
        ...
        for i in range(reach):
            ...
            if (pos[1] == lower_bound):
                pos = np.array([pos[0], upper_bound])
    ...
    def _get_closest_left(self):
        reach = self.size // 2
        pos_to_left = self._agent_location
        for pos in range(reach):
            pos_to_left = self.position_to_left(pos_to_left,1)
            if self.reward_matrix[pos_to_left[0]][pos_to_left[1]] == 3:
                return 0
            if self.reward_matrix[pos_to_left[0]][pos_to_left[1]] == 14:
                return 1
        return 2
    def _get_closest_right(self):
        reach = self.size // 2
        pos_to_right = self._agent_location
        for pos in range(reach):
            pos_to_right = self.position_to_right(pos_to_right,1)
            if self.reward_matrix[pos_to_right[0]][pos_to_right[1]] == 3:
                return 0
            if self.reward_matrix[pos_to_right[0]][pos_to_right[1]] == 14:
                return 1
        return 2
    def _get_closest_up(self):
        reach = self.size // 2
        pos_to_up = self._agent_location
        for pos in range(reach):
            pos_to_up = self.position_to_up(pos_to_up,1)
            if self.reward_matrix[pos_to_up[0]][pos_to_up[1]] == 3:
                return 0
            if self.reward_matrix[pos_to_up[0]][pos_to_up[1]] == 14:
                return 1
        return 2
    def _get_closest_down(self):
        reach = self.size // 2
        pos_to_down = self._agent_location
        for pos in range(reach):
            pos_to_down = self.position_to_down(pos_to_down,1)
            if self.reward_matrix[pos_to_down[0]][pos_to_down[1]] == 3:
                return 0
            if self.reward_matrix[pos_to_down[0]][pos_to_down[1]] == 14:
                return 1
        return 2
    ...
    def reset(self, seed=None, options=None, exec=False):
        ...
        self.total_reward = 0
        self.reward_matrix = [[0 for _ in range(self.size)] for _ in range(self.size)]
        self.reward_matrix[1][-1] = 10
        ...
        while np.array_equal(self._target_location, self._bdiamond_location):
            self._bdiamond_location = self.np_random.integers(0, self.size, size=2, dtype=int)
        ...
        self.closest_left = self._get_closest_left()
        self.closest_right = self._get_closest_right()
        self.closest_up = self._get_closest_up()
        self.closest_down = self._get_closest_down()
    ...
    def step(self, action):
        ...
        if np.array_equal(self._agent_location, self._ydiamond_location):
            self.reward_matrix[self._ydiamond_location[0]][self._ydiamond_location[1]] = 0
            self.reward_matrix[self._bdiamond_location[0]][self._bdiamond_location[1]] = 0
