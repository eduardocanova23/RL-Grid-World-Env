...
class GridWorldEnv4(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "time": spaces.Discrete(max_steps),
            }
        )
    ...
    def _get_obs(self):
        return {"agent": self._agent_location, "time": self._time} # , "target": self._target_location
    ...
    def step(self, action):
        ...
        if self._time == self.max_steps - 1:
        ...
        if shouldexplore:
            ...
            if random.uniform(0, 1) < self.exploration_max:
                ...
                for a in range(4):
                    ...
                    if self.table[self._agent_location[0], self._agent_location[1], self._time, a] == 0:
    ...
    def train_QLearning(self, n_episodes=1000000):
        ...
        for e in range(n_episodes):
            ...
            while (not self.terminated) and steps <= self.max_steps:
                ...
                self.table[self._agent_location[0],self._agent_location[1],self._time,action] = ((self.learning_rate*(reward+self.gamma*self.maxTable() 
                - self.table[self._agent_location[0],self._agent_location[1],self._time,action]))
                 + self.table[self._agent_location[0],self._agent_location[1],self._time,action])  
