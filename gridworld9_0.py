...
class GridWorldEnv9_0(gym.Env):
    ...
    def __init__(self,render_mode=None,size=4,exploration_max=0.90,exploration_min=0,exploration_decay=1.0,gamma=1,max_steps=18,learning_rate=1):
        ...
        self.reward_matrix = np.array([[-1,-1,-1,3],[-1,-1,-1,10], [-1, -1,-1,-1],[14, -1,-1,-1]])
        self.diamond_matrix = np.array([[-1,-1,-1,"b"],[-1,-1,-1,"g"], [-1, -1,-1,-1],["y", -1,-1,-1]])
    ...
    def reset(self, seed=None, options=None, exec=False):
        ...
        self._bdiamond_location = np.array([0,3])
    ...
    def step(self, action):
        ...
        if np.array_equal(self._agent_location, np.array([0,3])):
            self.reward_matrix[0,3] = -1
            self.reward_matrix[3,0] = -1
            self.diamond_matrix[3,0] = -1
            self.diamond_matrix[0,3] = -1
